...
# Read input file
covar <- read.table("spk_bamlist_r2_pca.covMat", stringsAsFact=FALSE);

# Read annot file
annot <- read.csv("spk_r2.csv", header = FALSE); # note that plink cluster files are usually tab-separated instead
#annot <- annot[-c(1),]
...
PC$Pop <- factor(PC$Tra, levels = c("Amargosa", "Owens", "Long Valley", "Lahontan"))
...
pca1 <- PC %>% ggplot(aes_string(x=x_axis, y=y_axis,color="Pop")) +  #geom_mark_ellipse(aes(fill = Group, group = Group),expand = unit(5, "mm"), con.border = "none") + 
  ...
  scale_shape_manual(values=c(17, 7, 9, 18)) + 
...
pca2 <- PC %>% ggplot(aes_string(x=x_axis, y=y_axis,color="Pop")) +  #geom_mark_ellipse(aes(fill = Group, group = Group),expand = unit(5, "mm"), con.border = "none") + 
  geom_point(size = 3, alpha = 0.8) +
  scale_shape_manual(values=c(17, 7, 9, 18)) + 
  scale_color_manual(values = c("#463730", "#94524A","#666309", "#ba931e" ))+
  ggtitle(title) + theme_void() +
...
value %>% ggplot(aes(x=num, y = val)) + geom_bar(stat = "identity") +
  theme_void()+
  labs(title="Genetic Variation Explained by PC1-PC30 in Group 2", y ="", x = "PC") +
  theme(axis.text = element_text(size = 10, face = "bold"),
